function [u, v, A, B, lambda2_y, run_stats] = init_sequential_do_vem(y, ...
                                                      u, v, A, B, ...
                                                      lambda2_y, k1, ...
                                                      k2, m1, m2, y_held)
%INIT_SEQUENTIAL_DO_VEM Perform vEM on the model for initialising subsections
%   Detailed explanation goes here
    ...
    while ~converged
        ...
        % Main steps
        fprintf('\nE-step\n')
        % The e-step is just like the regular case
        [pars, mf_pars] = do_e_step(old_model_pars, pars, mf_pars, fq);
        
        fprintf('\nM-step\n')
        % The m-step has to be altered so that we just update the
        % variables of interest and retain the already learned
        % variables fixed.
        model_pars = init_sequential_do_m_step(old_model_pars, pars, 30);
