function [u, A, B, lambda2_y, run_stats] = zm_do_vem(y, u, A, B, lambda2_y, k1, k2, m1, m2, y_held)
...
%   Detailed explanation goes here
    ...
    old_model_pars = zm_pack_model_pars(u, A, B, lambda2_y);
    ...
    fq = zm_get_model_free_energy(old_model_pars, pars, mf_pars);
    
    score = zm_get_validation_error(old_model_pars, pars, y_held, d_held);
    ...
    hist_score(1) = score;
    hist_iter(1) = 0;
    hist_model_pars{1} = zm_pack_to_var_array(u, A, B, lambda2_y);
    while ~converged
        iter = iter + 1;
        ...
        % Main steps
        fprintf('\nE-step\n')
        [pars, mf_pars] = zm_do_e_step(old_model_pars, pars, mf_pars, fq);
        
        fprintf('\nM-step\n')
        model_pars = zm_do_m_step(old_model_pars, pars, 35);
        ...
        fprintf('\nValidation error\n')
        score_old = score;
        score = zm_get_validation_error(model_pars, pars, y_held, d_held);
...
%         check_free_energy_increase(fq, fq_old);
        converged = zm_check_convergence(model_pars, old_model_pars, fq, fq_old, score, score_old);
        ...
        hist_model_pars{iter + 1} = zm_pack_to_var_array(u, A, B, lambda2_y);
